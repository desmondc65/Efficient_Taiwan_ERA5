...
\IEEEauthorblockA{Team 46\\
{\footnotesize\href{https://github.com/desmondc65/Efficient_Taiwan_ERA5}{Code (GitHub)}~\quad\textbullet\quad~\href{https://drive.google.com/drive/folders/1QEMg1qwU_Q34a9YN-VlyJhQIvSiUIZ3p}{Dataset (Google Drive)}}}%
}
...
plus \texttt{q,t,u,v,z} at 1000/850/500/250\,hPa; full list in
Appendix~\ref{app:vars}) on a $224\times128$
...
dependent). It provides raw, analysis-ready fields for all 24 atmospheric
variables (Appendix~\ref{app:vars}) --- usable directly for a range of
regional weather and ML workloads, with the \texttt{full\_field} layout
training out of the box. The release includes the Zarr store, the full
benchmark/rechunking toolkit, per-result hardware fingerprints, and every
metric row behind the tables and figures in this
paper.\footnote{Dataset download:
\url{https://drive.google.com/drive/folders/1QEMg1qwU_Q34a9YN-VlyJhQIvSiUIZ3p}.
Code, configuration, and checksums:
\url{https://github.com/desmondc65/Efficient_Taiwan_ERA5}.}
...
For full-field training reads, \texttt{full\_field} chunks scale to $6.1\times$
...
\section{Available Variables}\label{app:vars}
The released store provides 24 ERA5 channels as raw, analysis-ready
\texttt{float32} arrays on the shared $224\times128$ Taiwan grid: four
single-level (surface) fields and five upper-air fields sampled at four
pressure levels each. All channels are stacked along a single
\texttt{channel} axis in the order below.
\begin{center}\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}llll@{}}
\toprule
short name & variable & level(s) & unit \\
\midrule
\multicolumn{4}{@{}l}{\emph{Surface / single-level}}\\
\texttt{mslp} & Mean sea-level pressure & surface & Pa \\
\texttt{t2m}  & 2\,m temperature        & surface & K \\
\texttt{u10}  & 10\,m eastward wind     & surface & m\,s$^{-1}$ \\
\texttt{v10}  & 10\,m northward wind    & surface & m\,s$^{-1}$ \\
\midrule
\multicolumn{4}{@{}l}{\emph{Upper-air (each at 1000 / 850 / 500 / 250\,hPa)}}\\
\texttt{q\{lev\}} & Specific humidity & 1000/850/500/250 & kg\,kg$^{-1}$ \\
\texttt{t\{lev\}} & Temperature       & 1000/850/500/250 & K \\
\texttt{u\{lev\}} & Eastward wind     & 1000/850/500/250 & m\,s$^{-1}$ \\
\texttt{v\{lev\}} & Northward wind    & 1000/850/500/250 & m\,s$^{-1}$ \\
\texttt{z\{lev\}} & Geopotential      & 1000/850/500/250 & m$^2$\,s$^{-2}$ \\
\bottomrule
\end{tabular}
\end{center}
\noindent Channel order: \texttt{mslp, t2m, u10, v10}, then \texttt{q, t, u,
v, z} each at 1000/850/500/250\,hPa (i.e.\ \texttt{q1000}, \texttt{q850},
\dots, \texttt{z250}), 24 channels in total. Latitude spans
$21.6^\circ$--$25.6^\circ$N and longitude $119.75^\circ$--$122.25^\circ$E,
hourly.
